	...
	private void startGame() {
		...
		for (int c = 0; c < Globals.BRICKS_ROWS; c++) {
			for (int r = 0; r < Globals.BRICKS_COLUMNS; r++) {
				bricks[n] = new Brick(Globals.SCREEN_WIDTH * 0.05 + r * Brick.width,
						Globals.SCREEN_HEIGHT * 0.05 + c * Brick.height, 0.8f / (float) Globals.BRICKS_ROWS * c);
				add(bricks[n]);
				n++;
			}
		}
		...
		for (int i = 0; i < Globals.BALL_COUNT; i++) {
			var b = new Ball(paddle, i);
			balls.add(b);
			add(b);
		}
		...
		for (Ball ball : balls) {
			if ((ball.getBounds(null).intersects(paddle.getBounds(null)))) {
				...
				if (ballCenter >= paddleLeft && ballCenter < paddleLeft + paddle.getWidth() / 3) {
					ball.setxVelocity(-1);
				} else if (ballCenter >= paddleLeft + paddle.getWidth() / 3
						&& ballCenter < paddleLeft + paddle.getWidth() / 3 * 2) {
					ball.setxVelocity(0);
				} else {
					ball.setxVelocity(1);
				}
